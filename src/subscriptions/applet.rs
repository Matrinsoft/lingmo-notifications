...
pub async fn setup_panel_conn(tx: Sender<Input>) -> Result<Connection> {
    ...
    let conn = tokio::time::timeout(
        ...
        Builder::socket(socket)
            ...
            .unwrap()
            .serve_at(
                "/com/lingmoos/LingmoNotificationsSocket",
...
#[interface(name = "com.lingmoos.LingmoNotificationsSocket")]
    ...
    async fn get_fd(&self) -> zbus::fdo::Result<OwnedFd> {
        ...
        tokio::spawn(async move {
            let conn = match Builder::socket(mine).p2p().server(guid).unwrap().serve_at(
                "/com/lingmoos/LingmoNotificationsApplet",
                NotificationsApplet {
                    tx: tx_clone.clone(),
                },
            ) {
...
#[interface(name = "com.lingmoos.LingmoNotificationsApplet")]
